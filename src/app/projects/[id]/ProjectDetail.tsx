...
function L2SignalsDetails() {
  const stats = [
    { k: "BACKTEST", v: "76.9% WR · PF 4.0", s: "200 signals · real Dukascopy" },
    { k: "HYBRID EOD", v: "74.2% · PF 5.12", s: "+103R net · limit+stop fills" },
    { k: "LIVE DEMO", v: "$100 → $1,007", s: "37 days · 1–2% risk" },
    { k: "EXECUTION", v: "21:00 UTC", s: "hard close · 50%@+1R → BE" },
  ];
  return (
    <>
      <div
        className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-10 mb-6"
      >
        {stats.map((s) => (
          <div
            key={s.k}
            ...
            style={{
              background: "linear-gradient(135deg, rgba(0,201,120,0.10) 0%, var(--bg-primary) 100%)",
          ...
          >
            ...
            <div
              ...
              style={{ fontFamily: "'JetBrains Mono', monospace", color: "#00c978" }}
            >
              {s.v}
            </div>
            <div
      ...
      <div className="space-y-6">
        <SectionCard title="1. Signal Engine — 3 Strategies, One Discipline">
          <p>
            L2 runs a <strong style={{ color: "var(--accent-primary)" }}>3-tier 15M stack</strong> on XAU/USD: EMA bounce, consolidation breakout, and trend continuation (+ ORB on opens). Direction-aware routing gates entries on price vs EMA20 (1H); violations soften confidence instead of blocking, so the engine degrades gracefully instead of going blind.
          </p>
          <SubHeading>Hybrid execution</SubHeading>
          <p>
            Every signal ships <strong style={{ color: "var(--accent-primary)" }}>limit + stop legs with identical SL/TP</strong> — whichever level touches first fills, the other is cancelled, with a stop-fallback at 2.0× ATR. Trades run to the 21:00 UTC hard close; the exit playbook takes 50% at +1R, moves SL to breakeven, and rides the rest to TP.
          </p>
        </SectionCard>

        <SectionCard title="2. Data Pipeline — Real Feed, PC-Free Uptime">
          <p>
            Candles come from the <strong style={{ color: "var(--accent-primary)" }}>real Dukascopy XAU/USD feed</strong> (no synthetic data), written by a dedicated Render worker into a shared Firestore cache with cooldowns and freshness guards — the web app is read-only and never hammers the feed. A 1-minute auto-scorer grades every triggered signal from first-touch chronology (full win / directional ≥+1R / loss / expired-flat), so the ledger is engine-measured, not hand-waved.
          </p>
        </SectionCard>

        <SectionCard title="3. Business — Proof Converts to Revenue">
          <p>
            The verifiable ledger <em>is</em> the go-to-market: a trading community pays for signal access (sustaining the engine), while the same track record lands high-ticket client work — <strong style={{ color: "var(--accent-primary)" }}>Signals API access, custom trading systems</strong> built on client strategies or from scratch, <strong style={{ color: "var(--accent-primary)" }}>business websites and fintech apps</strong>, plus long-term broker and prop-firm partnerships.
          </p>
          <div className="flex flex-wrap gap-3 pt-2">
            <a
              href="https://l2signals-landing.vercel.app/services"
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs font-bold py-2.5 px-5 rounded-xl transition-all"
              style={{ fontFamily: "'Inter', sans-serif", background: "linear-gradient(135deg, #00c978, #00a854)", color: "#fff" }}
              onMouseEnter={(e) => { e.currentTarget.style.opacity = "0.9"; }}
              onMouseLeave={(e) => { e.currentTarget.style.opacity = "1"; }}
            >
              Services — API, Systems, Apps →
            </a>
            <a
              href="https://l2signals-landing.vercel.app/quote"
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs font-bold py-2.5 px-5 rounded-xl transition-all"
              style={{ fontFamily: "'Inter', sans-serif", border: "1px solid var(--border-color-strong)", color: "var(--accent-primary)" }}
            ...
            <a
              href="https://t.me/l2signals"
              target="_blank"
              rel="noopener noreferrer"
              className="text-xs font-bold py-2.5 px-5 rounded-xl transition-all"
              style={{ fontFamily: "'Inter', sans-serif", border: "1px solid var(--border-color-light)", color: "var(--text-muted)" }}
              onMouseEnter={(e) => { e.currentTarget.style.borderColor = "var(--accent-secondary)"; e.currentTarget.style.color = "var(--accent-primary)"; }}
      ...
      <div className="max-w-4xl mx-auto px-6 py-24">
        ...
        >
          {p.id === "calfit"
            ...
            : p.id === "l2-signals"
              ? "A systematic XAU/USD intraday signal business — 3-strategy 15M engine with hybrid limit+stop execution, backtested on real Dukascopy data (76.9% WR / PF 4.0) and forward-validated live (demo $100→$1,007.41 in 37 days). Verifiable trade ledger, Signals API access, and custom trading-system builds for clients."
              : p.description}
        </p>
        ...
        {p.id === "l2-signals" && <L2SignalsDetails />}
      </div>
    </main>
  );
}
